...
def analizza_storico_h2h(team_id_1, team_id_2, headers, url_base):
    querystring = {"action": "get_H2H", "firstTeamId": team_id_1, "secondTeamId": team_id_2}
    try:
        response = requests.get(url_base, headers=headers, params=querystring)
        dati = response.json()
        partite = dati if isinstance(dati, list) else []
        if not partite: return "Nessun dato H2H."
        tot_gol = sum(int(p.get('match_hometeam_score', 0) or 0) + int(p.get('match_awayteam_score', 0) or 0) for p in partite[:3])
...
try:
    ...
    resp = requests.get(url, headers=headers, params={"action": "get_events", "match_live": "1"})
    partite = resp.json()
    ...
    if isinstance(partite, list):
        ...
        for m in partite:
            ...
            # 2. FILTRI ORIGINARI
                ...
                segnale, consiglio = "⚽ GOL IMMINENTE", "👉 PUNTA: OVER 0.5 LIVE (2°T)"
                motivo_allerta = f"📊 Totali: Tiri {tiri_tot} | Attacchi {attacchi}"
            elif 48 <= minuto <= 80 and tiri_tot >= 15 and tiri_porta >= 4:
                segnale, consiglio = "⚡ ASSALTO MOSTRUOSO", "👉 PUNTA: OVER GOL / SEGNA GOAL"
                motivo_allerta = f"📊 Totali: Tiri {tiri_tot} (In porta {tiri_porta})"

            if segnale:
                h2h = analizza_storico_h2h(m.get('match_hometeam_id'), m.get('match_awayteam_id'), headers, url)
                messaggio = (
                    f"🔔 *SEGNALE: {segnale}*\n"
                    f"⚽ {casa} vs {ospite}\n"
